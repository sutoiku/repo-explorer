...
var safeSatisfy = function(repositoryVersion, depVersion, packageName) {
  try {
    return semver.satisfies(repositoryVersion, depVersion);
  } catch (e) {
    console.error('Unable to check if version %s satisfies %s for package %s : ', repositoryVersion, depVersion, packageName, e)
  }
}
  ...
  for (var i = 0; i < dependenciesNameToCheck.length; ++i) {
    ...
    if (safeCompare("lt", depVersion, repositoryVersion)) {
      ...
      info = "FUTURISTIC!!!";
               ...
               safeSatisfy(repositoryVersion, depVersion, dependencyName)) {
      info = "UP TO DATE";
    } else {
      info = "N/A";
